...
function onLoaded() {
	...
	allImage.forEach(function(image) {
		if (image.src == DEFAULT_IMAGE_URI) {
			TinyNews.loadImage(image.getAttribute("zhimg-src"));
		}
	});
}
...
function onImageClick(pImage) {
	...
	if (pImage.src == DEFAULT_LOADING_IMAGE_URI) {
		TinyNews.clickToLoadImage(pImage.getAttribute("zhimg-src"));
	} else {
		TinyNews.openImage(pImage.getAttribute("zhimg-src"));
	}
};
